...
import { TableNodeConfigPanel } from './TableNodeConfigPanel';
...
export function ProceduralNodeConfigEnhanced({ nodeType, initialData, onSave, onEdgeSync }: ProceduralNodeConfigProps) {
  ...
  const renderTableConfig = () => {
    // Use current initialData which gets updated from parent
    const [tableState, setTableState] = useState(initialData);
    ...
    const handleTableUpdate = (updates: any) => {
      ...
      // Directly call onSave for table updates to maintain real-time sync
      onSave(newState);
    };

    const handleRowsUpdate = (newRows: number) => {
      const currentCells = tableState.cells || [];
      let newCells = [...currentCells];
      
      if (newRows > currentCells.length) {
        // Add rows
        const colCount = tableState.cols || 3;
        for (let i = currentCells.length; i < newRows; i++) {
          newCells.push(new Array(colCount).fill(''));
        }
      } else if (newRows < currentCells.length) {
        // Remove rows
        newCells = newCells.slice(0, newRows);
      }
      
      const newRowHeights = new Array(newRows).fill(30);
      handleTableUpdate({ 
        rows: newRows, 
        cells: newCells,
    ...
    const handleColsUpdate = (newCols: number) => {
      const currentCells = tableState.cells || [];
      const newCells = currentCells.map(row => {
        if (newCols > row.length) {
          return [...row, ...new Array(newCols - row.length).fill('')];
        } else {
          return row.slice(0, newCols);
        }
      });
      
      const newColumnWidths = new Array(newCols).fill(80);
      handleTableUpdate({ 
        cols: newCols, 
        cells: newCells,
    ...
    return (
      <div className="space-y-6">
        <div>
          <label className="block text-sm font-medium mb-2">Table Label *</label>
          <Input
            value={tableState.label || ''}
            onChange={(e) => handleTableUpdate({ label: e.target.value })}
            placeholder="e.g., Data Table"
          />
        </div>

        <TableNodeConfigPanel
          rows={tableState.rows || 3}
          cols={tableState.cols || 3}
          config={tableState.config || {}}
          onUpdateRows={handleRowsUpdate}
          onUpdateCols={handleColsUpdate}
          onUpdateConfig={(config) => handleTableUpdate({ config })}
        />
      </div>
    );
  };
  ...
  const renderConfigForm = () => {
    switch (nodeType) {
      ...
      case 'table': return renderTableConfig();
